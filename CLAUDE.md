...
Models route through the LiteLLM proxy configured in `.env`. Set
`ANTHROPIC_BASE_URL`, `ANTHROPIC_AUTH_TOKEN`, and model overrides via
`settings.txt` (generated by `./generate-settings`).

To auto-detect models: `./generate-settings` then `source settings.txt`.
...
├── settings.example.txt   # Manual template
